...
bool FileFormatMP3::SetPosition(unsigned int seconds)
{
    ...
    if( seekFrame >= 0 )
    {
        qDebug() << "FileFormatMP3::Seeking to frame " << seekFrame;
        mpg123_seek_frame(_mpg123, seekFrame, SEEK_SET);
        return true;
    }
    else
    {
        qDebug() << "Error seeking frame: " << seekFrame;
        return false;
    }
}

bool FileFormatMP3::CanSetPosition()
{
    return true;
}
